...
export const useSimulatorStore = defineStore("simulator", {
  state: () => ({
    ...
    character: {
      ...
      addonLevels: Object.fromEntries(
        classList.map((cls) => [cls, [0, 0, 0, 0]]),
    ...
    opSlots: {
      A: [...Array(8)].map((_, i) => ({ id: `A-${i}`, name: "", isEX: i < 3 })),
      B: [...Array(8)].map((_, i) => ({ id: `B-${i}`, name: "", isEX: i < 3 })),
      C: [...Array(8)].map((_, i) => ({ id: `C-${i}`, name: "", isEX: i < 3 })),
    },
    fixaLevels: {
      A: 1,
      B: 1,
